...
Original file is located at
    https://colab.research.google.com/drive/1jebc3p72bEY6MQ1-7Lv1CQ7YXs1E4OZn
...
cs_id = "guy_lutsker"  # @param {type:"string"}
...
def load_dataset(filenames, batch_size, corruption_func, crop_size):
    ...
    """
    #    which outputs data_generator, a python generator object that outputs random tuples of the form
    #   (source_batch, target_batch), where each output variable is an array of shape (batch_size, height,
    #    width, 1), target_batch is made of clean images, and source_batch is their respective randomly
    #   corrupted version according to corruption_func(im). Each image in the batch should be picked at
    #   random from the given list of filenames, loaded as a grayscale image in the [0,1] range (you should use
    #   read_image which is available for you to use in Colab), followed by corrupting the entire image with
    #   corruption_func(im), and finally randomly choosing the location of a patch the size of crop_size, and
    #   subtract the value 0.5 from each pixel in both the source and target image patches

    s1, s2 = crop_size
    choose_idx = lambda n, m: (np.random.choice(n, size=1, replace=True)[0],
                               np.random.choice(m, size=1, replace=True)[0])
    file_name_dict = {}
    while True:
        file_names_indices = np.random.choice(len(filenames), size=batch_size, replace=True)

        corrupted_patches, original_patches = np.zeros((batch_size, s1, s2, 1)), \
                                              np.zeros((batch_size, s1, s2, 1))

        for i, idx in enumerate(file_names_indices):
            file_name_str = str(filenames[idx])
            if file_name_str not in file_name_dict:
                file_name_dict[file_name_str] = read_image(file_name_str, 1)
            img = file_name_dict[file_name_str]

            buff_h, buff_w = (s1 * 3), (s2 * 3)
            B_PTC_idx1, B_PTC_idx2 = choose_idx(img.shape[0] - buff_h + 1, img.shape[1] - buff_w + 1)

            B_ptch_org = img[B_PTC_idx1:B_PTC_idx1 + buff_h, B_PTC_idx2:B_PTC_idx2 + buff_w]
            B_ptch_corr = corruption_func(B_ptch_org)

            S_ptch_idx1, S_ptch_idx2 = choose_idx(B_ptch_org.shape[0] - s1 + 1, B_ptch_org.shape[1] - s2 + 1)

            S_ptch_org = B_ptch_org[S_ptch_idx1:S_ptch_idx1 + s1, S_ptch_idx2:S_ptch_idx2 + s2]
            S_ptch_corr = B_ptch_corr[S_ptch_idx1:S_ptch_idx1 + s1,
                          S_ptch_idx2:S_ptch_idx2 + s2]

            corrupted_small_patch_normalized, original_small_patch_normalized = \
                S_ptch_corr - 0.5, S_ptch_org - 0.5

            corrupted_small_patch_normalized_corrected = corrupted_small_patch_normalized[..., np.newaxis]
            original_small_patch_normalized_corrected = original_small_patch_normalized[..., np.newaxis]

            corrupted_patches[i] = corrupted_small_patch_normalized_corrected
            original_patches[i] = original_small_patch_normalized_corrected
        # print(corrupted_patches.shape)
...
def resblock(input_tensor, num_channels):
    ...
    """
    # Let X be our input, we first feed X to a 3×3 convolution,
    # followed by ReLU activation, then another
    # 3×3 convolution (this time no ReLU) and denote it’s output with O.
    # The final output of the residual block is ReLU(O + X)

    b = Conv2D(num_channels, (3, 3), padding='same', activation="relu")(input_tensor)
    O = Conv2D(num_channels, (3, 3), padding='same')(b)
    return Activation('relu')(Add()([input_tensor, O]))
...
def build_nn_model(height, width, num_channels, num_res_blocks):
    ...
    b = Conv2D(num_channels, (3, 3), padding='same', activation="relu")(a)
    d = b
    for i in range(num_res_blocks):
        d = resblock(d, num_channels)
    e = Conv2D(1, (3, 3), padding='same')(d)
    f = (Add()([a, e]))
    return Model(inputs=a, outputs=f)
...
def train_model(model, images, corruption_func, batch_size, steps_per_epoch, num_epochs, num_valid_samples):
    ...
    """
    t, v = train_test_split(images, test_size=0.2)
    crop = model.layers[0].input_shape[0][1:3]
    dataset_train = load_dataset(t, batch_size, corruption_func, crop)
    dataset_test = load_dataset(v, batch_size, corruption_func, crop)

    model.compile(loss='mean_squared_error', optimizer=Adam(beta_2=0.9))

    model.fit_generator(dataset_train, steps_per_epoch=steps_per_epoch, epochs=num_epochs,
...
def restore_image(corrupted_image, base_model):
    ...
    """
    a = Input(shape=(corrupted_image.shape[0], corrupted_image.shape[1],
                     1))  # with width and height of the image to be restored.
    b = base_model(a)
    new_model = Model(inputs=a, outputs=b)
    # print("shapes:")
    # print(corrupted_image.shape)
    new = corrupted_image[..., np.newaxis][np.newaxis, ...]
    # print(new.shape)
    # new_model.summary()
    res = new_model.predict(new - 0.5)
    return np.clip(res.squeeze() + 0.5, 0, 1).astype("float64")
...
def add_gaussian_noise(image, min_sigma, max_sigma):
    ...
    """
    # print("hi im corupting")
    sig = np.random.uniform(min_sigma, max_sigma, 1)
    noise = np.random.normal(0, sig, image.shape[0] * image.shape[1]).reshape(image.shape[0], image.shape[1])
    # print("     in corrupting:")
    # print("     orig img shape:" + str(image.shape))
    # print("     noise shape:" + str(noise.shape))
    noised = image + noise
    rounded = np.round((noised) / (1 / 255)) * (1 / 255)
    return np.clip(rounded, 0, 1)
...
def learn_denoising_model(denoise_num_res_blocks, quick_mode=False):
    ...
    ims = images_for_denoising()
    model = build_nn_model(24, 24, 48, denoise_num_res_blocks)
    corrupt = lambda x: add_gaussian_noise(x, 0, 0.2)
    bs, spe, ne, nvs = (100, 100, 10, 1000) if not quick_mode else (10, 3, 2, 30)
    train_model(model, ims, corrupt, bs, spe, ne, nvs)
    return model
...
def add_motion_blur(image, kernel_size, angle):
    ...
    """
    motion_ker = motion_blur_kernel(kernel_size, angle)
    return convolve(image, motion_ker, mode='constant')
...
def random_motion_blur(image, list_of_kernel_sizes):
    ...
    :return: blurred image
    """
    ang = np.random.uniform(0, np.pi, 1)[0]
    idx = int(np.random.uniform(0, len(list_of_kernel_sizes)))
    size = list_of_kernel_sizes[idx]
    blurred = add_motion_blur(image, size, ang)
    rounded = np.round((blurred) / (1 / 255)) * (1 / 255)
    return np.clip(rounded, 0, 1)
...
def learn_deblurring_model(deblur_num_res_blocks, quick_mode=False):
    ...
    :return: the trained model
    """
    ims = images_for_deblurring()
    model = build_nn_model(16, 16, 32, deblur_num_res_blocks)
    corrupt = lambda x: random_motion_blur(x, [7])
    bs, spe, ne, nvs = (100, 100, 10, 1000) if not quick_mode else (10, 3, 2, 30)
    train_model(model, ims, corrupt, bs, spe, ne, nvs)
    return model


m = learn_denoising_model(5, True)
m.save("models_modell_2")

model = learn_denoising_model(5, True)  # keras.models.load_model("models\modell_2")
ims = images_for_denoising()[0:5]
l = []
i = 0
for fname in ims:
    img = read_image(fname, 1)
    # plt.imshow(img)
    corr = add_gaussian_noise(img, 0, 0.2)
    l.append(corr)
    plt.imshow(corr, cmap='gray')
    plt.savefig('plot_corr_' + str(i) + '.png')
    # print(corr.shape)
    # corr = corr[...,np.newaxis]
    # corr = corr[np.newaxis,...]
    # print(corr.shape)
    clean = restore_image(corr, model)
    l.append(clean)
    plt.imshow(clean, cmap='gray')
    plt.savefig('plot_clean_' + str(i) + '.png')
    i += 1
    # print("hh")
    # plt.imshow(clean)

ig, ax = plt.subplots(nrows=len(ims), ncols=2)

i = 0
for row in ax:
    for col in row:
        col.imshow(l[i], cmap='gray')
        i += 1

plt.show()

# m = learn_deblurring_model(5,False)
# m.save("models\modell_3_blur")

# model = keras.models.load_model("models\modell_3_blur")
# ims = images_for_deblurring()[0:5]
# l = []
# i=0
# for fname in ims:
#   img = read_image(fname,1)
#   # plt.imshow(img)
#   corr = random_motion_blur(img,[7])
#   l.append(corr)
#   plt.imshow(corr, cmap = 'gray')
#   plt.savefig('plot_corr_motion_'+str(i)+'.png')
#   # print(corr.shape)
#   # corr = corr[...,np.newaxis]
#   # corr = corr[np.newaxis,...]
#   # print(corr.shape)
...
#   l.append(clean)
#   plt.imshow(clean, cmap = 'gray')
#   plt.savefig('plot_clean_motion_'+str(i)+'.png')
#   i+=1
#   # print("hh")
#   # plt.imshow(clean)


# ig, ax = plt.subplots(nrows=len(ims), ncols=2,figsize=(30,30))
...
def super_resolution_corruption(image):
    ...
    mesh = lambda x: np.meshgrid(np.arange(x[0]), np.arange(x[1]))
    map = lambda a, b: scipy.ndimage.map_coordinates(a, b, order=1, prefilter=False, mode='mirror')
    fix_up = lambda c: c.T.flatten()
    scale = lambda im, lam: zoom(im, lam, order=1, prefilter=False)

    list_of_zooms = [1 / i for i in range(2, 5)]
    n = random.choice(list_of_zooms)

    zoom_small, zoom_large = n, 1 / n
    zoomed_bad = scale(image, zoom_small)
    zoomed_back_bad = scale(zoomed_bad, zoom_large)

    x_temp, y_temp = mesh(image.shape)

    zoomed_back_good = map(zoomed_back_bad, np.vstack((fix_up(x_temp), fix_up(y_temp))))

    return zoomed_back_good.reshape(image.shape[0], image.shape[1])
...
def learn_super_resolution_model(super_resolution_num_res_blocks, quick_mode=False):
    ...
    :return: the trained model
    """
    ims = images_for_super_resolution()
    model = build_nn_model(16, 16, 32, super_resolution_num_res_blocks)
    corrupt = lambda x: super_resolution_corruption(x)
    bs, spe, ne, nvs = (100, 100, 10, 1000) if not quick_mode else (10, 3, 2, 30)
    train_model(model, ims, corrupt, bs, spe, ne, nvs)
    return model


# m = learn_super_resolution_model(5,False)
# m.save("models\modell_3_sp")

# model = keras.models.load_model("models\modell_3_sp")
# ims = images_for_super_resolution()[0:5]
# l = []
# i=0
# for fname in ims:
#   img = read_image(fname,1)
#   # plt.imshow(img)
#   corr = super_resolution_corruption(img)
#   l.append(corr)
#   plt.imshow(corr, cmap = 'gray')
#   plt.savefig('plot_corr_sp_'+str(i)+'.png')
#   # print(corr.shape)
#   # corr = corr[...,np.newaxis]
#   # corr = corr[np.newaxis,...]
#   # print(corr.shape)
...
#   l.append(clean)
#   plt.imshow(clean, cmap = 'gray')
#   plt.savefig('plot_clean_sp_'+str(i)+'.png')
#   i+=1
#   # print("hh")
#   # plt.imshow(clean)
...
def plot_me_up(model, images, corr_f, name, idx):
    ims = images()[0:5]
    l = []
    i = 0
    for fname in ims:
    ...
    plt.savefig(str(name) + "_blocks_" + str(idx) + '.png')
...
def effect_of_depth():
    ...
    """
    moodel_noise = lambda b: learn_denoising_model(b, False)
    moodel_blurr = lambda b: learn_deblurring_model(b, False)
    moodel_sp = lambda b: learn_super_resolution_model(b, False)
    models = {"noise": moodel_noise, "blurr": moodel_blurr, "sp": moodel_sp}
    images = {"noise": images_for_denoising, "blurr": images_for_deblurring,
              "sp": images_for_super_resolution}
    corr = {"noise": lambda x: add_gaussian_noise(x, 0, 0.2), "blurr": lambda x: random_motion_blur(x, [7]),
            "sp": lambda x: super_resolution_corruption(x)}
    for model in models.keys():
        for i in range(1, 10):
            m = models[model](i)
            m.save("models\model_" + str(model) + "with_" + str(i) + "_blocks")
            plot_me_up(m, images[model], corr[model], model, i)
